...
{
    struct twotubes chans = *(struct twotubes*)arg;
    while(1) {
...
static void
paintit(struct twotubes *chans, cairo_t *cr, int repcount)
{
...
{
    struct twotubes *chans = (struct twotubes*)data;
    double x1, y1, x2, y2;
    int repcount = 0;
    int rc;
...
int main(int argc, char **argv)
{
    ...
    chans.one = channel_create(sizeof(struct mandel_request), QSIZE, CHANNEL_LOCK_FREE_IMP);
    if(chans.one == NULL) {
    ...
    chans.two =
        channel_create(sizeof(struct mandel_reply),
                       synchronous ? 0 : QSIZE, CHANNEL_LOCK_FREE_IMP);
    if(chans.two == NULL) {
        perror("channel_create");
        exit(1);
    }
