...
int channel_send(struct channel *channel, const void *data)
{
	...
	{
		//TODO
		while(!OSAtomicCompareAndSwapIntBarrier(2, 3, &channel->volatileStoredDataNum))
		{
			printf("channel_send %d waits recv's address. Num=%d\n", channel->id, channel->volatileStoredDataNum);
			sleep(1);
		}
		printf("channel_send %d %p <- %p\n", channel->id, channel->data, data);
		memcpy(channel->data, data, channel->eltSize);
		if(OSAtomicCompareAndSwapIntBarrier(3, 4, &channel->volatileStoredDataNum))
		{
			printf("channel_send %d notify that data has been copied. Num=%d\n", channel->id, channel->volatileStoredDataNum);
		}
		rt = 1;
...
int channel_recv(struct channel *channel, void *data)
{
	...
	{
		//TODO
		while(!OSAtomicCompareAndSwapIntBarrier(0, 1, &channel->volatileStoredDataNum))
		{
			printf("channel_recv %d waits previous data to be copied. Num=%d\n", channel->id, channel->volatileStoredDataNum);
			sleep(1);
		}

		printf("channel_recv %d %p <- %p. Num=%d\n", channel->id, channel->data, data, channel->volatileStoredDataNum);
		channel->data = data;

		if(OSAtomicCompareAndSwapIntBarrier(1, 2, &channel->volatileStoredDataNum))
		{
			printf("channel_recv %d notify address has been set. Num=%d\n", channel->id, channel->volatileStoredDataNum);
		}

		while(!OSAtomicCompareAndSwapIntBarrier(4, 0, &channel->volatileStoredDataNum))
		{
			printf("channel_recv %d ensures that data has been copied. Num=%d\n", channel->id, channel->volatileStoredDataNum);
			sleep(1);
		}
		printf("channel_recv %d exits. Num=%d\n", channel->id, channel->volatileStoredDataNum);
